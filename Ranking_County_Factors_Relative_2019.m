...
for ii=1:length(S_ID)
    ...
    z_t=log(County_Trust_in_Science_v(ii,:)./(1-County_Trust_in_Science_v(ii,:)));
    z_new=pchip(Data_Year,z_t,Year_Baseline);
    t_new=1./(1+exp(-z_new));
    Baseline(ii)=t_new;
end

Scale_Trust_Science=1-(County_Trust_in_Science-min(Baseline))./(max(Baseline)-min(Baseline));
...
Scale_Trust_Medicine=1-(County_Trust_in_Medicine-min(Baseline))./(max(Baseline)-min(Baseline));
...
for jj=1:length(S_ID)
    ...
    z_new=pchip(Data_Year,z_temp(jj,:),Year_Baseline);
    Baseline(jj)=1./(1+exp(-z_new));
end
...
vb_demo=1./(1+exp(-pchip([2019 2022],log([0.88 0.91]./(1-[0.88 0.91])),Year_Baseline)));
vb_rep=1./(1+exp(-pchip([2019 2022],log([0.89 0.83]./(1-[0.89 0.83])),Year_Baseline)));
Baseline=vb_demo.*Baseline+vb_rep.*(1-Baseline);

Scale_MMR_Benefit_P=1-(County_MMR_Benefit-min(Baseline))./(max(Baseline)-min(Baseline));
...
CC_county=interp1([0 0.5 1],CC_temp,Scale_Norm);
    ...
    print(gcf,['Average_Ranking_Year=' num2str(Year_Plot) '_Baseline='  num2str(Year_Baseline) '.png'],'-dpng','-r300');
