...
Immunization_Variables={'DTaP','Polio','MMR','VAR'};
...
elseif(strcmp(Var_Name,'Trust_in_Medicine'))
    ...
    [~,~,Data_Year]=Stratified_Trust_in_Science_Medicine_County(Factor_M{1},County_ID);

    X_t=zeros(length(Factor_M),length(County_ID),length(Data_Year));

    for jj=1:length(Factor_M)        
        [~,County_Trust_in_Medicine_v,Data_Year]=Stratified_Trust_in_Science_Medicine_County(Factor_M{jj},County_ID);
...
elseif(strcmp(Var_Name,'Parental_Trust_in_Medicine'))
    ...
    County_Factor_v=Overall_State_County_Trust_Parental(county_weight,X_t,beta_z_Medicine,pop_interest,Year_Q);    
...
elseif(strcmp(Var_Name,'Parental_Trust_in_Science'))
    ...
    County_Factor_v=Overall_State_County_Trust_Parental(county_weight,X_t,beta_z_Science,pop_interest,Year_Q);    
