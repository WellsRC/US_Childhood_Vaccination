...
for yy=1:length(Year_Data)
    ...
    tf=strcmp(T_VN.Label,'Estimate!!Total!!AGE!!Under 5 years') | strcmp(T_VN.Label,'Total!!Estimate!!AGE!!Under 5 years') | strcmp(T_VN.Label,'Estimate!!Total!!Total population!!AGE!!Under 5 years');
    cv=T_VN.ColumnName(tf);
    tf=strcmp(cv,T.Properties.VariableNames);
    temp_total_under_5=table2array(T(:,tf));
    ...
    if(yy==1)
        ...
        temp_total_under_5_v=NaN.*zeros(length(County_Demo.County_ID),length(Year_Data));
    ...
    for zz=1:length(County_Demo.County_ID)
        ...
        if(sum(t_cindx)>0)           
            temp_total_under_5_v(zz,yy)=temp_total_under_5(t_cindx);
...
temp_O=NaN.*zeros(length(County_Demo.County_ID),length(table_year));
...
for yy=1:length(table_year)
   for cc=1:length(county_fips_u)
        ...
        if(sum(tf)>0)
            ...
            if(sum(t_o)>0)
                temp_O(tf,yy)=sum(TS.candidatevotes(t_o))./unique(TS.totalvotes(t_o));
            end
        end        
    end
end
...
Other_v=NaN.*zeros(length(County_Demo.County_ID),length(Year_Data));
...
for yy=1:length(Year_Data)
    ...
    temp_under_5=temp_total_under_5_v(:,yy);
    ...
    temp_65_plus=temp_total_65_plus_v(:,yy);
    ...
    temp_male_65_plus=temp_male_65_plus_v(:,yy);
    ...
    temp_female_65_plus=temp_female_65_plus_v(:,yy);
    ...
    if(Year_Data(yy)<2017)
        ...
        temp_male_65_plus=(temp_male_65_plus./100).*temp_male_total_v(:,yy);
        ...
        temp_female_65_plus=(temp_female_65_plus./100).*temp_female_total_v(:,yy);
    end
    ...
    County_Demo.Population.Age_under_5(:,yy)=temp_under_5;
...
County_Demo.Political.Democratic=squeeze(Democratic_v(:,:))./temp_pol;
County_Demo.Political.Republican=squeeze(Republican_v(:,:))./temp_pol;
County_Demo.Political.Other=squeeze(Other_v(:,:))./temp_pol;
