...
MMR_RE=readtable([pwd '\Vaccination_Data\State_Exemptions.xlsx'],'Sheet','MMR_Religious');
Vac_RE=readtable([pwd '\Vaccination_Data\State_Exemptions.xlsx'],'Sheet','Other_Religious');

MMR_PE=readtable([pwd '\Vaccination_Data\State_Exemptions.xlsx'],'Sheet','MMR_Philosophical');
Vac_PE=readtable([pwd '\Vaccination_Data\State_Exemptions.xlsx'],'Sheet','Other_Philosophical');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
...
for yy=2017:2023
    ...
    for jj=1:height(MMR_RE)
        tf=MMR_RE.State_FIPs(jj)==[str2double(State_FP)];
        MMR_Religious_Exemption(tf)=MMR_RE.(['Year_' num2str(yy)])(jj);
        tf=MMR_PE.State_FIPs(jj)==[str2double(State_FP)];
        MMR_Philosophical_Exemption(tf)=MMR_PE.(['Year_' num2str(yy)])(jj);
        tf=Vac_RE.State_FIPs(jj)==[str2double(State_FP)];
        Other_Religious_Exemption(tf)=Vac_RE.(['Year_' num2str(yy)])(jj);
        tf=Vac_PE.State_FIPs(jj)==[str2double(State_FP)];
        Other_Philosophical_Exemption(tf)=Vac_PE.(['Year_' num2str(yy)])(jj);
    end    
