...
X_State=zeros(length(Yr).*length(State_ID),length(Var_N));  
X_County=zeros(length(Yr).*length(County_ID),length(Var_N));  

for yy=1:length(Yr)
    for jj=1:length(Var_N)
        if(strcmp(Var_N{jj},'Trust_in_Medicine') || strcmp(Var_N{jj},'Trust_in_Science'))
            X_State((yy-1).*length(State_ID)+[1:length(State_ID)],jj)=Return_State_Data(Var_N{jj},Yr(yy),State_ID);
            X_County((yy-1).*length(County_ID)+[1:length(County_ID)],jj)=Return_County_Data(Var_N{jj},Yr(yy),County_ID);
        else
            [State_Demo,Data_Year]=Demographics_State(Var_N{jj},State_ID);
            X_State((yy-1).*length(State_ID)+[1:length(State_ID)],jj)=State_Demo(:,Data_Year==Yr(yy));
            [County_Demo,Data_Year]=Demographics_County(Var_N{jj},County_ID);
            X_County((yy-1).*length(County_ID)+[1:length(County_ID)],jj)=County_Demo(:,Data_Year==Yr(yy));
        end
    end
end

X_State=[log(X_State(:,[1 2])./(1-X_State(:,[1 2]))) log(X_State(:,[3])) log(X_State(:,[4:length(Var_N)])./(1-X_State(:,[4:length(Var_N)])))];
X_County=[log(X_County(:,[1 2])./(1-X_County(:,[1 2]))) log(X_County(:,[3])) log(X_County(:,[4:length(Var_N)])./(1-X_County(:,[4:length(Var_N)])))];
...
beta_v=zeros(length(Inqv),(num_model-1),2+length(Var_N));
...
for dd=1:length(Inqv)
    ...
    lb=-300.*ones(1,2+length(Var_N));
    lb(2+[6:7])=0; % Trust lower bound (Assume can only have postive influence on uptake)
    ub=300.*ones(1,2+length(Var_N));
    ub(1:2)=0;
...
writetable(T,'County_Level_Cross_Validation_Alt.xlsx','Sheet','Indicator');

Variables={'Religous Exemptions','Philosophical Exemptions','Economic','Education','Income','Politcal','Race','Trust in Medicine','Trust in Science','Uninsured under 19'};
for vv=1:length(Inqv)
    T=array2table(squeeze(beta_v(vv,:,:)));
    T.Properties.VariableNames=Variables;
    writetable(T,'County_Level_Cross_Validation_Alt.xlsx','Sheet',['Coefficients_' Inqv{vv} ]);
end
