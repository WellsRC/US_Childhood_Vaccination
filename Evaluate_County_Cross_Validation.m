...
%         Year{yy+(vv-1).*length(Year_Inq)}=[num2str(Year_Inq(yy)) char(8211) num2str(Year_Inq(yy)-2000)];
%         Vaccine{yy+(vv-1).*length(Year_Inq)}=Vac_Name{vv};
%     end
% end
% 
% Error=zeros(4.*length(Year_Inq),6);
...
%         Error(yy+(vv-1).*length(Year_Inq),:)=[mean(Diff_V) prctile(Diff_V,[2.5 25 50 75 97.5])];    
%     end
% end
% 
% T=table(Year,Vaccine,Error);
% % 
close all;
...
for yy=1:length(Year_Inq)
    for vv=1:4
        ...
        switch vv
            ...
            for mm=1:length(c_indx_error)
                text(ymin,mm-1,[num2str(c_bound_error(mm,1),'%3.0f') '%'],'HorizontalAlignment','center','Fontsize',16);
            end
            text(ymin,length(c_indx_error),[num2str(100,'%3.0f') '%'],'HorizontalAlignment','center','Fontsize',16);
            text(ymin+2.5,max(c_indx_error)./2,{[Vac_Title_v{vv} ' uptake error']},'HorizontalAlignment','center','Fontsize',18,'Rotation',270);
            axis off;    
