function [County_Trust_in_Science,County_Trust_in_Medicine,Data_Year]=Stratified_Trust_in_Science_Medicine_County(Var_1,County_ID)
    ...
    if(strcmp(Var_1,'Age'))
        for ii=1:length(County_ID)
            ...
            if(sum(tf)>0)
                Pop_temp=P.Age_18_34(tf,:)+P.Age_35_49(tf,:)+P.Age_50_64(tf,:)+P.Age_65_plus(tf,:);
                County_Trust_in_Science(ii,:)=(TRS.age_18_34.Great_Deal(ismember(Trust_Year_Data,Data_Year)).*P.Age_18_34(tf,:)+TRS.age_35_49.Great_Deal(ismember(Trust_Year_Data,Data_Year)).*P.Age_35_49(tf,:)+TRS.age_50_64.Great_Deal(ismember(Trust_Year_Data,Data_Year)).*P.Age_50_64(tf,:)+TRS.age_over_65.Great_Deal(ismember(Trust_Year_Data,Data_Year)).*P.Age_65_plus(tf,:));
               County_Trust_in_Science(ii,:)=County_Trust_in_Science(ii,:)+0.5.*(TRS.age_18_34.Only_Some(ismember(Trust_Year_Data,Data_Year)).*P.Range_18_to_34(tf,:)+TRS.age_35_49.Only_Some(ismember(Trust_Year_Data,Data_Year)).*P.Range_35_to_49(tf,:)+TRS.age_50_64.Only_Some(ismember(Trust_Year_Data,Data_Year)).*P.Age_50_64(tf,:)+TRS.age_over_65.Only_Some(ismember(Trust_Year_Data,Data_Year)).*P.Age_65_plus(tf,:));
                County_Trust_in_Science(ii,:)=County_Trust_in_Science(ii,:)./Pop_temp;

                County_Trust_in_Medicine(ii,:)=(TRM.age_18_34.Great_Deal(ismember(Trust_Year_Data,Data_Year)).*P.Age_18_34(tf,:)+TRM.age_35_49.Great_Deal(ismember(Trust_Year_Data,Data_Year)).*P.Age_35_59(tf,:)+TRM.age_50_64.Great_Deal(ismember(Trust_Year_Data,Data_Year)).*P.Age_50_64(tf,:)+TRM.age_over_65.Great_Deal(ismember(Trust_Year_Data,Data_Year)).*P.Age_65_plus(tf,:));
               County_Trust_in_Medicine(ii,:)=County_Trust_in_Medicine(ii,:)+0.5.*(TRM.age_18_34.Only_Some(ismember(Trust_Year_Data,Data_Year)).*P.Age_18_34(tf,:)+TRM.age_35_49.Only_Some(ismember(Trust_Year_Data,Data_Year)).*P.Age_35_59(tf,:)+TRM.age_50_64.Only_Some(ismember(Trust_Year_Data,Data_Year)).*P.Age_50_64(tf,:)+TRM.age_over_65.Only_Some(ismember(Trust_Year_Data,Data_Year)).*P.Age_65_plus(tf,:));
